...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        //only needs else case, otherwise n doeesnt change
        if (!m.isZero()) {
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        //if exponent is zero, power is 1
        if (p.isZero()) {
            n.copyFrom(one);
        }
        //case for if the exponent is even
        else if (isEven(p)) {
            tempp.divide(two);
            n.multiply(tempn);
            powerMod(n, tempp, m);
        }
        //case for if exponent is odd
        else {
            tempp.decrement();
            if (!tempp.isZero()) {
                tempp.divide(two);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber tempw = new NaturalNumber2();
        tempw.copyFrom(w);
        //first test case
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            while (witness) {
                //increment counter until halfway betwen 0-50
                counter++;
                if (counter > 25 || isPrime) {
                    witness = false;
                }
                //decrement tempn twice
                temp.decrement();
                temp.decrement();
                NaturalNumber rand = randomNumber(temp);

                temp.copyFrom(n);
                while (rand.compareTo(two) <= 0 || rand.compareTo(temp) == 0) {
                    temp.decrement();
                    temp.decrement();
                    //gets random number in range
                    rand = randomNumber(temp);
                    temp.increment();
                    temp.increment();

                }
                //tests if test value is a composite of n
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
         ...
         */
        //if n is even ,gets next odd number
        NaturalNumber two = new NaturalNumber2(2);
        if (isEven(n)) {
            n.increment();
        }
        //else adds two until n is a likely prime number, will stay odd if only adding 2
        while (!isPrime2(n)) {
            n.add(two);
        }
