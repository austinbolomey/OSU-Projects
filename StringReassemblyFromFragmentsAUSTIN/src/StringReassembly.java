...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //concats the non overlaped portion of str2 with all of str1
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        Set<String> bye = strSet.newInstance();
        boolean found = false;
        //if empty set, add string
        if (strSet.size() == 0) {
            strSet.add(str);
            //otherwise
        } else {
            ...
            for (String element : strSet) {
                //if temp set doenst have element and substring contains the element,remove element
                if (!bye.contains(element) && str.indexOf(element) >= 0) {
                    bye.add(element);
                }
                //if element contains the new string, found becomes true
                if (element.indexOf(str) >= 0) {
                    found = true;
                }
            }
            //removes all elements that are substrings of the new string
            strSet.remove(bye);
            //if no element in the set contains new string, add string to set
            if (!found) {
                strSet.add(str);
            }
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while (!input.atEOS()) {
            //adds line by line through method
            String line = input.nextLine();

            addToSetAvoidingSubstrings(returnset, line);
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //replaces all ~ with \n,and outputs through simplewriter
        String temp = text;
