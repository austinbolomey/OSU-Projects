...
public class List3<T> extends ListSecondary<T> {
    ...
    private void createNewRep() {
        ...
        this.rightLength = 0;
        //prestart node
        Node smartPreStart = new Node();
        this.preStart = smartPreStart;

        //post finish node
        Node smartPostFinish = new Node();
        this.postFinish = smartPostFinish;
    ...
    public final void addRightFront(T x) {
        ...
        addingNode.data = x;
        //add new node between lastLeft and lastLeft.next
        addingNode.next = this.lastLeft.next;
        this.lastLeft.next.previous = addingNode;
        addingNode.previous = this.lastLeft;

        this.lastLeft.next = addingNode;
    ...
    public final T removeRightFront() {
        ...
        T rightFrontData = removedNode.data;
        //sets previous of the next node after removed to lastleft
        removedNode.next.previous = this.lastLeft;
        //return the data of the rightFront

        //set last left next to the next of the removed node
        this.lastLeft.next = removedNode.next;
        this.rightLength--;

        assert this.conventionHolds();
        // Fix this line to return the result after checking the convention.
        return rightFrontData;
    }
