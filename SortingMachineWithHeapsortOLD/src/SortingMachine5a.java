...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if (left <= last) {
            ...
            T leftNode = array.entry(left);
            int smallest = left;
            if (right <= last) {
                T rightNode = array.entry(right);
                if (order.compare(leftNode, rightNode) > 0) {
                    smallest = right;
                }
            }
            if (order.compare(array.entry(smallest), root) > 0) {
    ...
    public final T removeFirst() {
        ...
        this.heap.setEntry(0, this.heap.entry(this.heapSize - 1));
        this.heapSize--;
