...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if (last >= left) {
            ...
            if (order.compare(array.entry(top), array.entry(smaller)) > 0) {
                array.exchangeEntries(top, smaller);
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        //create array from queue length
        Array<T> result = new Array1L<>(q.length());
        int len = q.length();
        //populate array with queue elements
        for (int i = 0; i < len; i++) {
            result.setEntry(i, q.dequeue());
        }
        heapify(result, 0, order);
    ...
    public final T removeFirst() {
        ...
        //create a temp array 1 element smaller than heap
        Array<T> tempRemove = new Array1L<>(this.heapSize - 1);
        T removed = this.heap.entry(0);
        //loop through until 1 less than the size of the heap and add every element from the
        //intial heap into the new heap, skipping the first element of inital heap
        for (int i = 0; i < this.heapSize - 1; i++) {
            tempRemove.setEntry(i, this.heap.entry(i + 1));
        }
        //replace inital heap with temp heap
        this.heap.transferFrom(tempRemove);
        //re-heapify
        heapify(this.heap, 0, this.machineOrder);
        //adjust final heap size
        this.heapSize--;
        assert this.conventionHolds();
        return removed;
    }
    ...
    public final int size() {
        ...
        int size = 0;
        //different cases for insertion mode/extraction mode
        if (this.insertionMode) {
