...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        int bucket = this.mod(key.hashCode(), this.hashTable.length());
        //adds value to correct bucket location
    ...
    public final Pair<K, V> remove(K key) {
        ...
        Map.Pair<K, V> removed = this.hashTable.entry(bucket).remove(key);
        this.size--;
        return removed;
    }
    ...
    public final boolean hasKey(K key) {
        ...
        int bucket = mod(key.hashCode(), this.hashTable.length());
        if (this.hashTable.mayBeExamined(bucket)) {
